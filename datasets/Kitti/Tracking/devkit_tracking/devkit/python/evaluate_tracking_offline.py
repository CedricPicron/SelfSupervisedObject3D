...
def evaluate(gt_dir, result_dir, mail):
    """
    ...
    for c in ("car", "pedestrian"):
        e = trackingEvaluation(gt_dir, result_dir, mail=mail, cls=c)
...
# input:
#   - gt_dir (groundtruth label dir)
#   - result_dir (result label dir)
if __name__ == "__main__":

    # check for correct number of arguments
    if len(sys.argv)!=3:
      print "Usage: python evaluate_tracking_offline.py gt_dir result_dir"
      sys.exit(1);
    ...
    # evaluate results
    mail = mailpy.Mail("")
    success = evaluate(gt_dir, result_dir, mail)
