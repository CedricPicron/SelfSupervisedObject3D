...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos){
  ...
  for (int v = 0; v < 3; ++v)
      for (int i = 0; i < vals[v].size(); i = i + 1)
          sum[v] += vals[v][i];
  printf("%s AP: %f %f %f\n", file_name.c_str(), sum[0] / N_SAMPLE_PTS * 100, sum[1] / N_SAMPLE_PTS * 100, sum[2] / N_SAMPLE_PTS * 100);
  fprintf(fp,"\nAP: %f %f %f\n", sum[0] / N_SAMPLE_PTS * 100, sum[1] / N_SAMPLE_PTS * 100, sum[2] / N_SAMPLE_PTS * 100);
  fclose(fp);
  ...
  for (int32_t j=0; j<2; j++) {
    ...
    if (j==0) {
      fprintf(fp,"set term png size 450,315 font \"FreeSans\" 11\n");
      fprintf(fp,"set output \"%s.png\"\n",file_name.c_str());
    } else {
      fprintf(fp,"set term postscript eps enhanced color font \"FreeSans\" 20\n");
    ...
    system(command);
    sprintf(command,"cd %s; rm %s",dir_name.c_str(),(file_name + ".gp").c_str());
    system(command);
  }
  ...
  //system(command);
  //sprintf(command,"cd %s; pdfcrop --noverbose %s_large.pdf %s.pdf",dir_name.c_str(),file_name.c_str(),file_name.c_str());
  //system(command);
...
bool eval(string gt_dir, string result_dir, Mail* mail){
  ...
  string plots_dir       = result_dir + "/plots";
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      fp_det = fopen((stats_dir + "/stats_" + CLASS_NAMES[c] + "_detection.txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((stats_dir + "/stats_" + CLASS_NAMES[c] + "_orientation.txt").c_str(),"w");
      ...
      fclose(fp_det);
      saveAndPlotPlots(plots_dir, CLASS_NAMES[c] + "_detection", CLASS_NAMES[c], precision, 0);
      if(compute_aos){
        saveAndPlotPlots(plots_dir, CLASS_NAMES[c] + "_orientation", CLASS_NAMES[c], aos, 1);
        fclose(fp_ori);
      }
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      fp_det = fopen((stats_dir + "/stats_" + CLASS_NAMES[c] + "_detection_ground.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
      ...
      fclose(fp_det);
      saveAndPlotPlots(plots_dir, CLASS_NAMES[c] + "_detection_ground", CLASS_NAMES[c], precision, 0);
    }
  }
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_3d[c]) {
      fp_det = fopen((stats_dir + "/stats_" + CLASS_NAMES[c] + "_detection_3d.txt").c_str(), "w");
      vector<double> precision[3], aos[3];
      ...
      fclose(fp_det);
      saveAndPlotPlots(plots_dir, CLASS_NAMES[c] + "_detection_3d", CLASS_NAMES[c], precision, 0);
    }
  }
