...
''' =================================================================================== '''
with torch.no_grad():
    for sceneIndex in random.sample(range(751), k=args.numberOfSamples):
        ...
        with open(imagePathsFileName, 'r') as imagePathsFile:
            ...
            imagePaths = [line.split(' ')[-1] for line in imagePaths]

        for i, imagePath in enumerate(imagePaths):
            image = Image.open(os.path.join(args.dataRoot, imagePath))
        ...
        os.chdir(videosDir)
        subprocess.run(['ffmpeg', '-hide_banner', '-loglevel', 'panic', '-framerate', '2', '-i', '%06d.png', '{:d}.mp4'.format(sceneIndex)])
