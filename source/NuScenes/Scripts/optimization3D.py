...
calibDir = os.path.join(args.dataRoot, 'training/calib')
imageDir = os.path.join(args.dataRoot, 'training/image_02')
...
sceneIndices = list(range(751))
imageLabels = dict()
imageIndex = -1

for sceneIndex in sceneIndices:
    calib3x4 = readCalibrationFile(calibDir, 2, sceneIndex)
    invCalib3x3 = np.linalg.inv(calib3x4[:, :-1])
    
    imagePathsFileName = os.path.join(imageDir, '{:04d}.txt'.format(sceneIndex))
    with open(imagePathsFileName, 'r') as imagePathsFile:
        imagePaths = imagePathsFile.read().splitlines()

    for frameIndex, imagePath in enumerate(imagePaths):
        imageIndex += 1
        imageLabels[imageIndex] = []

        fullImagePath = os.path.join(args.dataRoot, imagePath.split(' ')[1])
        image = Image.open(fullImagePath).convert('RGB')
...
''' ================================================================================ '''
''' ------------------------- Create per image label files ------------------------- '''
''' ================================================================================ '''

imageLabelDir = os.path.join(args.experimentRoot, 'Results/Optimization3D/label_02')
if not os.path.exists(imageLabelDir):
    imageIndex = -1
    os.makedirs(imageLabelDir)
    sceneLabelDir = os.path.join(args.dataRoot, 'training/label_02')
    
    for sceneIndex in sceneIndices:
        imagePathsFileName = os.path.join(imageDir, '{:04d}.txt'.format(sceneIndex))
        with open(imagePathsFileName, 'r') as imagePathsFile:
            numberOfFrames = len(imagePathsFile.read().splitlines())

        labelFileName = os.path.join(sceneLabelDir, '{:04d}.txt'.format(sceneIndex))
        with open(labelFileName, 'r') as labelFile:
            lines = labelFile.read().splitlines()
        
        lineDict = {frameIndex: [] for frameIndex in range(numberOfFrames)}
        for line in lines:
            words = line.split(' ')
            frameIndex = int(words[0])
            lineDict[frameIndex].append(' '.join(words[2:]))

        for frameIndex in range(numberOfFrames):
            imageIndex += 1
            imageLabelFileName = os.path.join(imageLabelDir, '{:06d}.txt'.format(imageIndex))
            with open(imageLabelFileName, 'w') as imageLabelFile:
                for imageLine in lineDict[frameIndex]:
                    imageLabelFile.write(imageLine+'\n')
...
runOfflineEvaluation(imageLabelDir, imageLabels, resultDir)
...
for imageIndex, carLabels in imageLabels.items():
    gtLabels = readLabelFile(imageIndex, imageLabelDir)
